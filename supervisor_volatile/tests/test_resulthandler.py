import sys
import unittest
import supervisor_volatile
import supervisor_volatile.resulthandler
from supervisor_volatile.compat import _b, _u
...
class TestStdinWriteHandler(unittest.TestCase):
    def test_handler_does_nothing_when_response_is_OK(self):
        event = DummyEvent()
        response = 'OK'
        supervisor_volatile.resulthandler.stdin_write_handler(event, response)

    def test_handler_rejects_event_when_response_is_unexpected(self):
        event = DummyEvent()
        response = 'unexpected'
        self.assertRaises(RejectEvent,
                          supervisor_volatile.resulthandler.stdin_write_handler,
                          event, response)
